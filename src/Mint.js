...
const Mint = () => {
    ...
    const gaslessTxn = async () => {
        ...
        /**@todo @arpit29joshi - Replace Lumos address here */
        if (
            contractAddress.toLowerCase() ===
            "0xc51327f24f90b3521990db8a3075e1cedd3ccf42"
        ) {
            if (!email) {
            ...
            try {
                const fetchQueryString = `white-list-user/get-by-address?contractAddress=${contractAddress}&userAddress=${email}`;

                const res = await fetch(
                    `${process.env.REACT_APP_SERVER_URL}${fetchQueryString}`
                );

                if (!res.ok)
                    throw new Error(
                        String(res.status) + String(res.statusText)
                    );

                const { data } = await res.json();
                console.log("Whitelist user data: ", data);
                ...
                if (
                    data.whiteListUsers[0].start === data.whiteListUsers[0].max
                ) {
                    toast.error("You have minted maximum NFTs allowed!!");
                    return;
                }
                ...
                biconomy
                    .onEvent(biconomy.readOnlyProvider, async () => {
                        toast.loading(
                        ...
                        try {
                            ...
                            let txhash = await ethersProvider.send(
                                "eth_sendTransaction",
                                [txnn]
                            );
                ...
                console.log("Error in biconomy txn: ", err);
                toast.error(
                    "Something went wrong! Please check browser console for more info."
                );
            }
                            ...
                            <header className="heading-container">
                                ...
                                <div className="wallet-address-display">
                                    ...
                                    <div>
                                        ...
                                        <a
                                            href={`${constantsValues[chainToConnect].blockExplorer}${contractAddress}`}
                                            target={"/blank"}
                                        >
                                            <p className="text">
                                                Contract Details
                            ...
                            <div className="modal">
                                ...
                                <div className="content-container">
                                    ...
                                    <p className="congratulations-msg">
                                        ...
                                        minted {claimerDetails.title}, Check
                                        what you have got!!
                                    </p>
                                </div>
                                <div className="content-container">
                                    ...
                                    <a
                                        ...
                                        href={`${constantsValues[chainToConnect].openSeaNFTDetails}${contractAddress}/${mintDetails.totalSupply}`}
                                        target="_blank"
